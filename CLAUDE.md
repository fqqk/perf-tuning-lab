...
```
Phase 1: 問題分析（学習者が実施）
  ├─ ベースライン計測結果から「何が原因か」を考える
  └─ データから推論する力を養う

Phase 2: チューニング候補の検討（学習者が主体）
  ├─ 複数の対策案を考える
  ├─ 各案の「なぜ効くのか」を理由付けする
  ├─ トレードオフを判断する
  └─ どれを試すか自分で決める

Phase 3: 実装（学習者がコードを書く）
  ├─ 修正案を理解してから実装
  ├─ ドキュメント・コード例を参考に
  └─ 手を動かすことで理解を深める

Phase 4: 検証（学習者が実行・分析）
  ├─ 自分で k6 テスト実行
  ├─ ビフォーアフター比較
  ├─ 「期待と異なったか」を分析
  └─ なぜ期待と異なったのか考察する
...
| Phase | 学習者の行動 | 講師の行動 |
|-------|-----------|---------|
| **1. 問題分析** | 計測結果を読む、仮説を立てる | 問題を提示、データ解釈をガイド |
| **2. 候補検討** | 複数案を検討、理由を考える | 選択肢を提示、判断ポイント指摘 |
| **3. 実装** | コードを書く | 修正案ではなくヒント・リファレンス提示 |
| **4. 検証** | テスト実行、結果分析 | 結果解釈のガイド、深掘り質問 |
...
- [ ] 「同じ問題が実務で出たとき、対処できるか」
- [ ] 「なぜそのチューニングが効いたのか説明できるか」
- [ ] 「期待と異なった場合、原因を特定できるか」
- [ ] 「他の言語でも同じ対策が使えるか判断できるか」
